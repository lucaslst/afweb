<template>
  <div>
    ...
    <h1> Listar Apartamentos por Endereço </h1>
  </div>

  <div>
        Digite o Endereço que deseja visualizar: <input type="text" name="" id="endereco" v-model="endereco" /><br /><br />  
       
        <button @click="listarEndereco"> Listar Apartamentos </button><br /><br />
  ...
  methods: {
   ...
   listarEndereco: function() {
    if(this.endereco.length < 5)
        alert("Digite um endereço maior que 5 caracteres!");
    else{
      this.$http.get(this.baseURI + "/search?endereco=" + this.endereco).then((result) => {
        this.apartamentos = result.data;
      })
