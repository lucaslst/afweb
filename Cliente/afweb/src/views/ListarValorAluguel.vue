<template>
  <div>
    ...
    <h1> Listar Apartamentos por Valor de Aluguel (Menor) </h1>
  </div>

  <div>
        Digite o Valor de Aluguel que deseja visualizar: <input type="text" name="" id="valoraluguel" v-model="valoraluguel" /><br /><br />  
       
        <button @click="listarValorAluguel"> Listar Apartamentos </button><br /><br />
...
export default {
  ...
  methods: {
   ...
   listarValorAluguel: function() {
    if(this.valoraluguel <= 0)
        alert("Digite um valor maior que 0!");
    else{
      this.$http.get(this.baseURI + "/page?valoraluguel=" + this.valoraluguel).then((result) => {
        this.apartamentos = result.data;
      })
