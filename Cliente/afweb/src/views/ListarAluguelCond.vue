<template>
  <div>
    ...
    <h1> Listar Apartamentos por Valor de Aluguel e Condomínio (Menor e Maior) </h1>
  </div>
  ...
  <div>
        ...
        Digite o Valor mínimo de Condomínio que deseja visualizar: <input type="text" name="" id="menorcond" v-model="menorcond" /><br /><br />  
        Digite o Valor máximo de Condomínio que deseja visualizar: <input type="text" name="" id="maiorcond" v-model="maiorcond" /><br /><br />

        <button @click="listarAluguelCond"> Listar Apartamentos </button><br /><br />
...
export default {
  ...
  methods: {
   ...
   listarAluguelCond: function() {
    if(this.menoraluguel <= 0 || this.maioraluguel <=0 || this.menorcond <=0 || this.maiorcond <= 0)
        alert("Digite um valor maior que 0!");
    if(this.menoraluguel >= this.maioraluguel || this.menorcond >= this.maiorcond)
        alert("Digite corretamente o valor dos campos!")
    else{
      this.$http.get(this.baseURI + "/page3?menoraluguel="+this.menoraluguel+"&maioraluguel="+this.maioraluguel+"&menorcond="+this.menorcond+"&maiorcond="+this.maiorcond).then((result) => {
        this.apartamentos = result.data;
      })
